test.other.normalization <- function(date.input = '2011-11-04', date.checked.output = '2011-11-04'){
	...
	test.output.zscore      <- NanoStringNorm:::other.normalization(x[grepl('Endogenous', anno$Code.Class),], anno, otherNorm = 'zscore', verbose = FALSE);
	test.output.quantile    <- NanoStringNorm:::other.normalization(x[grepl('Endogenous', anno$Code.Class),], anno, otherNorm = 'quantile', verbose = FALSE);
	...
	check2.1 <- checkException(NanoStringNorm:::other.normalization(x[grepl('Endogenous', anno$Code.Class),], anno, otherNorm = 'garbage', verbose = FALSE));
